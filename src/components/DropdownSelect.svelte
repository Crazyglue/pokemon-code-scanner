...
<div class="relative w-64" use:clickOutside on:outclick={() => (open = false)}>
	...
	<button
		on:click={() => (open = !open)}
		class="relative w-50 rounded-md border border-gray-300 bg-white px-4 py-2 text-left shadow-sm focus:outline-none md:w-full"
	>
		<span class="block truncate pr-4">{selected ? selected : 'Select an option'}</span>
